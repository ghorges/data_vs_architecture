...
INTERACTION_DEFINITIONS = {
    "formula_x_pearson": ("frequent_formula_family_bucket", "frequent_pearson_symbol_bucket"),
    "formula_x_spacegroup": ("frequent_formula_family_bucket", "frequent_spacegroup_bucket"),
    "pearson_x_spacegroup": ("frequent_pearson_symbol_bucket", "frequent_spacegroup_bucket"),
    "pearson_x_wyckoff": ("frequent_pearson_symbol_bucket", "frequent_wyckoff_signature_bucket"),
    "formula_x_wyckoff": ("frequent_formula_family_bucket", "frequent_wyckoff_signature_bucket"),
    "spacegroup_x_wyckoff": ("frequent_spacegroup_bucket", "frequent_wyckoff_signature_bucket"),
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build reusable interaction buckets for the singleton high-risk background branch.",
    )
    parser.add_argument(
        "--min-count",
        type=int,
        default=10,
        help="Minimum background-branch count required for an interaction bucket to receive its own label.",
    )
    parser.add_argument(
        "--output-prefix",
...
def main() -> None:
    ...
    for interaction_name, (left_col, right_col) in INTERACTION_DEFINITIONS.items():
        raw_column = f"{interaction_name}_raw"
        bucket_column = f"frequent_{interaction_name}_bucket"
        output[raw_column] = background[left_col].astype(str) + "__" + background[right_col].astype(str)
        counts = (
        ...
        summary["interaction_buckets"][interaction_name] = {
            "selected_count": int(len(selected)),
            "coverage": float(counts.loc[counts["candidate_count"] >= args.min_count, "candidate_count"].sum() / len(background)),
            "top_selected": counts.loc[counts["candidate_count"] >= args.min_count].head(15).to_dict(orient="records"),
        }

    output_path_parquet = PROCESSED_DIR / f"{args.output_prefix}_component_interaction_proxy.parquet"
    output_path_csv = PROCESSED_DIR / f"{args.output_prefix}_component_interaction_proxy.csv"
    summary_path = PROCESSED_DIR / f"{args.output_prefix}_component_interaction_proxy_summary.json"
