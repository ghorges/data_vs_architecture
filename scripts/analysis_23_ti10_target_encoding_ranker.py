...
from analysis_22_ti10_failure_mode_decomposition import load_ti10_frame
...
SPACEGROUP_WYCKOFF_BASE = [
    "frequent_spacegroup_bucket",
    "frequent_wyckoff_signature_bucket",
]
FREQUENT_BUCKET_BASE = [
    "frequent_ti10_spacegroup_wyckoff_bucket",
]

FEATURE_SETS = [
    MixedFeatureSet(
        name="spacegroup_plus_wyckoff",
        categorical=SPACEGROUP_WYCKOFF_BASE,
        numeric=[],
    ),
    MixedFeatureSet(
        name="spacegroup_plus_wyckoff_plus_token_te",
        categorical=SPACEGROUP_WYCKOFF_BASE,
        numeric=["token_te_mean", "token_te_log_count"],
    ),
    MixedFeatureSet(
        name="spacegroup_plus_wyckoff_plus_full_token",
        categorical=[*SPACEGROUP_WYCKOFF_BASE, "frequent_prototype_token_bucket"],
        numeric=[],
    ),
    MixedFeatureSet(
        name="frequent_bucket",
        categorical=FREQUENT_BUCKET_BASE,
        numeric=[],
    ),
    MixedFeatureSet(
        name="frequent_bucket_plus_token_te",
        categorical=FREQUENT_BUCKET_BASE,
        numeric=["token_te_mean", "token_te_log_count"],
    ),
    MixedFeatureSet(
        name="frequent_bucket_plus_full_token",
        categorical=[*FREQUENT_BUCKET_BASE, "frequent_prototype_token_bucket"],
        numeric=[],
    ),
]


def load_branch_frame() -> pd.DataFrame:
...
def smoothed_target_encoding(
    train_tokens: pd.Series,
    y_train: np.ndarray,
    test_tokens: pd.Series,
    ...
    train_mean = train_tokens.map(lookup["token_te_mean"]).to_numpy(dtype=float)
    train_count = train_tokens.map(lookup["token_te_log_count"]).to_numpy(dtype=float)
    ...
    return np.column_stack([train_mean, train_count]), np.column_stack([test_mean, test_count])
...
def evaluate_branch_target_encoding(frame: pd.DataFrame, outcome: str) -> tuple[pd.DataFrame, pd.DataFrame]:
...
def compute_deltas(summary: pd.DataFrame) -> pd.DataFrame:
    ...
    out["n_materials"] = pivot[("n_materials", "spacegroup_plus_wyckoff")].to_numpy()
    out["positive_rate"] = pivot[("positive_rate", "spacegroup_plus_wyckoff")].to_numpy()

    comparisons = [
        (
            "spacegroup_plus_wyckoff",
            "spacegroup_plus_wyckoff_plus_token_te",
            "spacegroup_plus_wyckoff_plus_full_token",
            "spacegroup_base",
        ),
        (
            "frequent_bucket",
            "frequent_bucket_plus_token_te",
            "frequent_bucket_plus_full_token",
            "frequent_bucket_base",
        ),
    ]
    for base_name, te_name, full_name, prefix in comparisons:
        out[f"{prefix}_token_te_balanced_accuracy_gain"] = (
            pivot[("balanced_accuracy_mean", te_name)] - pivot[("balanced_accuracy_mean", base_name)]
        ).to_numpy()
        out[f"{prefix}_full_token_balanced_accuracy_gain"] = (
            pivot[("balanced_accuracy_mean", full_name)] - pivot[("balanced_accuracy_mean", base_name)]
        ).to_numpy()
        out[f"{prefix}_token_te_vs_full_token_balanced_accuracy_delta"] = (
            pivot[("balanced_accuracy_mean", te_name)] - pivot[("balanced_accuracy_mean", full_name)]
        ).to_numpy()
        out[f"{prefix}_token_te_average_precision_gain"] = (
            pivot[("average_precision_mean", te_name)] - pivot[("average_precision_mean", base_name)]
        ).to_numpy()
        out[f"{prefix}_full_token_average_precision_gain"] = (
            pivot[("average_precision_mean", full_name)] - pivot[("average_precision_mean", base_name)]
        ).to_numpy()
        out[f"{prefix}_token_te_vs_full_token_average_precision_delta"] = (
            pivot[("average_precision_mean", te_name)] - pivot[("average_precision_mean", full_name)]
        ).to_numpy()
        out[f"{prefix}_token_te_log_loss_improvement"] = (
            pivot[("log_loss_mean", base_name)] - pivot[("log_loss_mean", te_name)]
        ).to_numpy()
        out[f"{prefix}_full_token_log_loss_improvement"] = (
            pivot[("log_loss_mean", base_name)] - pivot[("log_loss_mean", full_name)]
        ).to_numpy()
        out[f"{prefix}_token_te_vs_full_token_log_loss_delta"] = (
...
def main() -> None:
    output_dir = RESULTS_DIR / "tables" / "analysis_23"
    ensure_dir(output_dir)

    frame = load_branch_frame()
    fold_tables: list[pd.DataFrame] = []
    summary_tables: list[pd.DataFrame] = []
    for outcome in OUTCOMES:
        folds, summary = evaluate_branch_target_encoding(frame=frame, outcome=outcome)
    ...
    summary = {
        "tI10_branch": {
            ...
            "key_deltas": {
                outcome: {
                    "spacegroup_base_token_te_balanced_accuracy_gain": float(
                        delta_lookup.loc[outcome, "spacegroup_base_token_te_balanced_accuracy_gain"]
                    ),
                    "spacegroup_base_full_token_balanced_accuracy_gain": float(
                        delta_lookup.loc[outcome, "spacegroup_base_full_token_balanced_accuracy_gain"]
                    ),
                    "spacegroup_base_token_te_vs_full_token_balanced_accuracy_delta": float(
                        delta_lookup.loc[outcome, "spacegroup_base_token_te_vs_full_token_balanced_accuracy_delta"]
                    ),
                    "spacegroup_base_token_te_average_precision_gain": float(
                        delta_lookup.loc[outcome, "spacegroup_base_token_te_average_precision_gain"]
                    ),
                    "spacegroup_base_full_token_average_precision_gain": float(
                        delta_lookup.loc[outcome, "spacegroup_base_full_token_average_precision_gain"]
                    ),
                    "spacegroup_base_token_te_vs_full_token_average_precision_delta": float(
                        delta_lookup.loc[outcome, "spacegroup_base_token_te_vs_full_token_average_precision_delta"]
                    ),
                    "frequent_bucket_base_token_te_balanced_accuracy_gain": float(
                        delta_lookup.loc[outcome, "frequent_bucket_base_token_te_balanced_accuracy_gain"]
                    ),
                    "frequent_bucket_base_full_token_balanced_accuracy_gain": float(
                        delta_lookup.loc[outcome, "frequent_bucket_base_full_token_balanced_accuracy_gain"]
                    ),
                    "frequent_bucket_base_token_te_vs_full_token_balanced_accuracy_delta": float(
                        delta_lookup.loc[outcome, "frequent_bucket_base_token_te_vs_full_token_balanced_accuracy_delta"]
                    ),
                    "frequent_bucket_base_token_te_average_precision_gain": float(
                        delta_lookup.loc[outcome, "frequent_bucket_base_token_te_average_precision_gain"]
                    ),
                    "frequent_bucket_base_full_token_average_precision_gain": float(
                        delta_lookup.loc[outcome, "frequent_bucket_base_full_token_average_precision_gain"]
                    ),
                    "frequent_bucket_base_token_te_vs_full_token_average_precision_delta": float(
                        delta_lookup.loc[outcome, "frequent_bucket_base_token_te_vs_full_token_average_precision_delta"]
                    ),
                }
                for outcome in OUTCOMES
            },
